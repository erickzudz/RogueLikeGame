...
{
    [Header("Refs")]
    public RoomsCorridorsDungeon dungeon;   // generador (instancia en escena)
    ...
    Transform container; // __SPAWNS__ bajo el dungeon

    void EnsureContainer()
    {
        if (!dungeon) { Debug.LogError("[SpawnPointPlacer] Falta 'dungeon'."); return; }
        if (!container)
        {
            var go = new GameObject("__SPAWNS__");
            container = go.transform;
            container.SetParent(dungeon.transform, false);  // SIEMPRE instancia de escena
    ...
    public void PlaceAll()
    {
        ...
        {
            foreach (var go in created) if (go)
                    if (Application.isPlaying) Destroy(go); else DestroyImmediate(go);
            created.Clear();

            foreach (var sp in container.GetComponentsInChildren<SpawnPoint>())
                if (sp) if (Application.isPlaying) Destroy(sp.gameObject); else DestroyImmediate(sp.gameObject);
        }
    ...
    void CreateSpawn(Vector2Int cell, SpawnPointType type, string name, float radius)
    {
        ...
        go.transform.SetParent(container, false);   // nunca a un prefab del Project
